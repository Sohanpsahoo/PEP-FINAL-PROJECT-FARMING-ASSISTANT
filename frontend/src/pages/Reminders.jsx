...
export default function Reminders() {
  ...
  const REMINDER_CATEGORIES = [
    ...
    { value: 'weather', label: 'Weather Alert', icon: '🌦️', gradient: 'from-cyan-400 to-blue-500', light: 'bg-cyan-50 border-cyan-200 text-cyan-700' },
  ...
  useEffect(() => {
    if (currentUser?.userId) fetchReminders(currentUser.userId);
  }, [currentUser, filter]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiFetch('/api/reminders/', {
        ...
        body: JSON.stringify({ ...formData, farmer: currentUser.userId, due_date: new Date(formData.due_date).toISOString() })
      });
      if (response.ok) {
        fetchReminders(currentUser.userId);
        setFormData({ title: '', description: '', due_date: '', category: 'operation', priority: 'medium' });
        ...
        <div className="relative bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 rounded-3xl p-8 mb-8 overflow-hidden shadow-xl shadow-orange-500/20">
          ...
          <div className="relative z-10 grid grid-cols-4 gap-4 mt-6">
            ...
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4 text-center border border-white/20">
              ...
              <div className="text-orange-100 text-sm font-medium">⚠️ Overdue</div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`bg-white rounded-3xl overflow-hidden shadow-sm border transition-all duration-500 hover:shadow-xl hover:-translate-y-2 group ${isOverdue ? 'border-red-200' :
      ...
      <div className="p-5">
        ...
        <div className={`inline-block px-3 py-1 rounded-full text-xs font-bold border mb-3 ${categoryInfo.light}`}>
          {categoryInfo.label}
        </div>
