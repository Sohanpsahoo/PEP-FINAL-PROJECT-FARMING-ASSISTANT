...
export default function Farms() {
  ...
  const IRRIGATION_TYPES = [
    { value: 'rain_fed', label: 'Rain Fed', icon: '🌧️' },
    ...
    { value: 'bore_well', label: 'Bore Well', icon: '🕳️' },
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 transition-all duration-300">
        ...
        <div className="relative bg-gradient-to-r from-emerald-600 via-green-600 to-teal-600 rounded-3xl p-8 mb-8 overflow-hidden shadow-xl shadow-emerald-500/20">
          ...
          {farms.length > 0 && (
            <div className="relative z-10 grid grid-cols-3 gap-4 mt-6">
              ...
              <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4 text-center border border-white/20">
                <div className="text-3xl font-extrabold text-white">{[...new Set(farms.flatMap(f => (f.primary_crops || '').split(',').map(c => c.trim())))].length}</div>
                <div className="text-green-100 text-sm font-medium">Crop Types</div>
              </div>
            </div>
          )}
        </div>
        ...
        {showAddForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" style={{ animation: 'fadeIn 0.3s ease-out' }}>
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-100" style={{ animation: 'slideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1)' }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input type="text" value={formData.primary_crops}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading && farms.length === 0 ? (
            ...
            farms.map((farm, index) => {
              ...
              const crops = (farm.primary_crops || '').split(',').map(c => c.trim()).filter(Boolean);
              ...
              return (
                <div key={farm.id} className="group bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-500 hover:-translate-y-2"
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    {/* Crops as tags */}
                    <div>
                      <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">🌾 Crops</div>
                      <div className="flex flex-wrap gap-2">
                        {crops.map((crop, i) => (
                          <span key={i} className="px-3 py-1.5 bg-gradient-to-r from-green-100 to-emerald-100 text-emerald-800 text-sm font-bold rounded-full border border-emerald-200 hover:scale-105 transition-transform cursor-default">
                            {crop}
                          </span>
                        ))}
                      </div>
